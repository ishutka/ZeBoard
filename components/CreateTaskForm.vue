				...
				>
					<template v-slot:item="{ item }">
						<div class="d-flex align-center">
							<img :alt="item.name" :src="item.avatar" class="avatar"/>
							{{ item.name }}
						</div>
					</template>
					<template v-slot:selection="{ item }">
						<div class="d-flex align-center">
							<img :alt="item.name" :src="item.avatar" class="avatar mb-0"/>
							{{ item.name }}
						</div>
					</template>
				</v-select>
...
export default {
	...
	methods: {
		...
		addTaskToLocalstorage(task) {
			const newColumn = {...this.toDoColumn};
			newColumn.tasks = [task, ...this.toDoColumn.tasks];
			window.localStorage.setItem("ZeBoard-column-TODO", JSON.stringify(newColumn));
			this.setToDoColumn(newColumn);
		},
